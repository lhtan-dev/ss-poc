...
class DynamoDbCommon(Evaluator):
    ...
    def __init__(self, tables, dynamoDbClient, cloudWatchClient, serviceQuotaClient, appScalingPolicyClient, backupClient):
        super().__init__()
        self.tables = tables
        self.dyanmoDbClient = dynamoDbClient
    ...
    def __check_delete_protection(self):
        try:
    ...
    def __check_resources_for_tags(self):
        try:
    ...
    def __check_unused_resources_gsi_read(self):
        try:
    ...
    def __check_unused_resources_gsi_write(self):
        try:
    ...
    def __check_attribute_length(self):
        try:
            for tableAttributes in self.tables['Table']['AttributeDefinitions']:
                if len(tableAttributes['TableName']) > 15:
                    ...
                    self.results['attributeNamesXL'] = [-1, table['Table']['TableName'] + ' - attribute name ' + tableAttributes['TableName']]
                elif len(tableAttributes['TableName']) > 8:
                    # error attributesNamesL for length > 8 <= 15
                    self.results['attributeNamesL'] = [-1, table['Table']['TableName'] + ' - attribute name ' + tableAttributes['TableName']]
        
        except botocore.exceptions.CLientError as e:
            raise e
        
    # logic to check for TTL status
    def __check_time_to_live_status(self):
        try:
    ...
    def __check_continuous_backup(self):
        try:
            ...
            if result['ContinuousBackupsDescription']['ContinuousBackupsStatus'] == 'DISABLED':                    
                self.results['continuous_backups'] = [-1, 'Continuous Backup Disabled for table ' + self.tables['Table']['TableName']]

        except botocore.exceptions.ClientError as e:
            raise e
    
    # logic to check autoscaling aggresiveness
    def __check_autoscaling_aggresiveness(self):
        try:
            results = self.applicationScalingClient.describe_scaling_policies(
            ...
            for eachScalingPolicies in results['ScalingPolicies']:
                if eachScalingPolicies is not None:
                    if eachScalingPolicies['TargetTrackingScalingPolicyConfiguration']['TargetValue'] <= 50:
                        self.results['autoScalingLowUtil'] = [-1, table['Table']['TableName'] + ' is on low utilization policy of value : ' + eachScalingPolicies['TargetTrackingScalingPolicyConfiguration']['TargetValue']]
                    elif eachScalingPolicies['TargetTrackingScalingPolicyConfiguration']['TargetValue'] >= 90:                         
                        self.results['autoScalingHighUtil'] = [-1, table['Table']['TableName'] + ' is on high utilization policy of value : ' + eachScalingPolicies['TargetTrackingScalingPolicyConfiguration']['TargetValue']]

        except botocore.exceptions as e:
            raise e
    
    # logic to check capacity mode
    def __check_capacity_mode(self):
        try:
            #Count the number active reads on the table on the GSIs
            result = self.cloudWatchClient.get_metric_statistics(
                Namespace = 'AWS/DynamoDB',
    ...
    def __check_autoscaling_status(self):
        try:
    ...
    def __check_backup_status(self):
        try:
